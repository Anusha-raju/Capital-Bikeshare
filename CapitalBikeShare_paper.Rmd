...
We use the `rpart` package in R to build a decision tree classifier. The target variable, member_casual, is predicted based on the following features:

`rideable_type`: Type of bike.

`distance_miles`: Distance traveled in miles.

`ride_duration_mins`: Duration of the ride in minutes.

`start_time_period`: Time period when the ride started (e.g., morning, afternoon).

`day_type`: Type of day (e.g., weekday or weekend).
